...
class HelpPageMixin(BasePageMixin):
    def _build_help_page(self) -> QScrollArea:
        ...
        for text in (
            "模式 1 · 体力条反弹：确认中鱼图标并追踪角色头顶体力条，槽中点先连续变灰，再确认填充宽度开始连续回升时收杆，对镜头和画面清晰度要求较高。若没有识别到反弹并出现跑鱼提示，会记录本轮上钩到跑鱼的时长 T；提前量为 T × 10%，但最少 1.0 秒、最多 2.0 秒，下一轮按 T − 提前量兜底。例如 14.0 秒跑鱼，会在 12.6 秒收杆。",
