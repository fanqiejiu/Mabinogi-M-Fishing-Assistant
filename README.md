...
<p align="center">Windows 本地钓鱼辅助工具 · v0.6.4</p>
...
- **OK 指定窗口模式（推荐）**：默认优先识别标题为“瑪奇 Mobile”的窗口；找不到时可手动选择。使用 WGC 截图和窗口消息，不会把按键发送给当前前台程序。
...
4. 默认使用“指定窗口后台模式（推荐）”。确认目标窗口是“瑪奇 Mobile”，再把鼠标放到圆形按钮中心，直接按 `F7`。
5. 按 `F9` 保存识别区域和完整诊断；完整画面只保存在本机，发送前请确认其中没有不想分享的内容。
6. 点击“开始监测”或按 `F8`。

如果设备不接受后台截图或按键，可切换“屏幕坐标模式（前台）”，回到游戏画面后重新校准一次。
