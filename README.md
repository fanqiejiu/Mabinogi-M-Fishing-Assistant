...
<p align="center">本地运行的画面识别钓鱼辅助工具 · v0.3.0 · by 番茄啾</p>
...
- **指定窗口后台模式（实验性）**：可选择洛奇 M 窗口；截图及 `Space` / `W` / `S` 仅发往该窗口，不会把按键发给当前正在使用的其他程序。
...
5. 默认使用“稳定屏幕模式”。若要试用后台模式，先将“识别目标”切换为“指定窗口后台模式（实验性）”，刷新并选择洛奇 M 窗口。
6. 将鼠标放在右下角圆形钓鱼按钮**正中心**，点击“校准当前鼠标位置”或按 `F7`。
7. 可选：按 `F9` 保存识别区域快照，确认圆形按钮未被截断。
8. 点击“开始监测”或按 `F8`。

稳定屏幕模式需要游戏位于前台。指定窗口后台模式校准完成后可以切换到其他窗口使用；洛奇 M 必须保持打开且**不可最小化**。

> 指定窗口后台模式采用 Windows 的窗口截图与定向键盘消息。不同 DirectX 渲染路径、游戏更新或安全机制可能忽略后台截图或按键；若提示兼容性错误或没有反应，请切回稳定屏幕模式。程序不会将按键回退发送到其他前台程序。
...
fishing_assistant/
  ...
  engine.py          # 识别流程、按键和全局快捷键
  splash.py          # 启动页
  system_profile.py  # 一次性硬件信息读取
  ui.py              # PySide6 桌面界面
  updates.py         # GitHub Release 更新检查
  window_target.py   # Win32 指定窗口截图与定向按键（实验性）
  assets/            # 项目图标
tests/               # 核心识别逻辑回归测试
```
