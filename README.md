...
<p align="center">Windows 本地钓鱼辅助工具 · v0.5.3.1</p>
...
1. 模式 1 改为严格的两阶段判断：绿色体力条第一次下降穿过半条只记录，反弹后第二次穿过半条才收杆。
2. 新增跑鱼提示识别。出现“猶豫了一下，結果讓牠跑掉了”时记录本轮耗时，下一轮识别仍失败时提前 1–2 秒兜底收杆。
3. 模式 1 标注为“实验性功能，不稳定”，界面会显示是否已经学习到跑鱼时间。
4. 后台模式加入窗口内虚拟悬停，真实鼠标可以继续操作浏览器或其他程序。
5. 修复打包版配置保存在临时目录、重启后可能丢失的问题。
